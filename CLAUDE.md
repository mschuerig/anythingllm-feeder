...
- **Settings precedence**, applied identically across both tools: CLI flag > env var > `config.json` > built-in default. New resolvers should follow this pattern. In ingest: `config.resolve_settings(url_override=, api_key_override=)`, `config.resolve_anythingllm_storage_dir(override=)`, `anythingllm.resolve_timeout(io_timeout_override=)`. The `--api-key-file` flag is read at CLI entry (`cli._resolve_api_key_file`) and merged into `args.api_key` so downstream code only sees a string. Inline `--api-key` triggers a one-line stderr security note (`note: --api-key is visible in shell history ...`), suppressed under `-q`.
- forage's whisper extractor is per-model cached: `extractors.get_whisper(model=None)` keys on the string `"mlx-whisper"` (so existing test stubs work); `get_whisper(model="…")` keys on the tuple `("mlx-whisper", model)`. `transcribe._resolve_whisper_model` returns `None` when the resolved model equals the extractor's built-in `DEFAULT_MODEL` so the default flow always hits the string-keyed cache entry.
